...
def load_data_and_create_splits(
    ...
    train_data_ratio: float,
    splits_prefix: str,
    cols_to_keep: Optional[dict[str, list[str]]] = None,
):
    ...
    cols_to_keep : dict[str, list[str]], optional
        An optional mapping of columns to keep from each file, where
        keys need to correspond to ``transactions_filename`` and ``identity_filename``
    ...
    def read_csv_with_pyarrow(
        file_path: str, cols_to_keep: Optional[list[str]] = None
    ) -> pa.Table:
        """Read CSV file using PyArrow with native S3 support."""

        read_options = pa_csv.ReadOptions(use_threads=True)
        parse_options = pa_csv.ParseOptions(delimiter=",")
        convert_options = pa_csv.ConvertOptions(include_columns=cols_to_keep)

        # Prepare file source for local or S3
        if file_path.startswith("s3://"):
            ...
            file_source = s3_fs.open_input_stream(s3_path)
        else:
            # For local files, read from the source directly
    ...
    # Read transaction data using PyArrow, optionally selecting columns
    if cols_to_keep and transactions_filename in cols_to_keep:
        transaction_cols_to_keep = cols_to_keep[transactions_filename]
    else:
        transaction_cols_to_keep = None

    transaction_df = read_csv_with_pyarrow(
        os.path.join(raw_data_path, transactions_filename), transaction_cols_to_keep
    ).to_pandas()
    ...
    # Read identity data using PyArrow, optionally selecting columns
    if cols_to_keep and identity_filename in cols_to_keep:
        identity_cols_to_keep = cols_to_keep[identity_filename]
    else:
        identity_cols_to_keep = None
    identity_df = read_csv_with_pyarrow(
        os.path.join(raw_data_path, identity_filename), identity_cols_to_keep
    ).to_pandas()
...
def get_relations_and_edgelist(
    ...
    for card_num in range(1, 7):  # card1 through card6
        ...
        card_vertices = encode_featureless_nodes(card_vertices)
        write_compatible_csv(
            card_vertices,
    ...
    productcd_vertices = encode_featureless_nodes(productcd_vertices)
    write_compatible_csv(
        productcd_vertices,
    ...
    for addr_num in range(1, 3):  # addr1 and addr2
        ...
        addr_vertices = encode_featureless_nodes(addr_vertices)
        write_compatible_csv(
            addr_vertices,
    ...
    for email_type in ["P_emaildomain", "R_emaildomain"]:
        ...
        email_vertices = encode_featureless_nodes(email_vertices)
        write_compatible_csv(
            email_vertices,
...
def encode_featureless_nodes(node_df: pd.DataFrame):
    """Encode node IDs into embeddings as input node features.

    This function adds text embeddings for featureless nodes, so that models trained on a graph
    can be used for inductive inference, i.e., predicting on unseen nodes.

    To meet Neptune Database import file format, the 384D embeddings will be converted into a
    string in which float values are separated by semicolumns, like "0.45232331;-0.5673872;...".

    Parameters
    ----------
    node_df: DataFrame
        A DataFrame that contains two columns, "~id", and "~label".

    Returns
    -------
    node_df: DataFrame
        The same DataFrame, but has a new column, named 'id_embeddings'.
    """
    ...
    # Convert each embedding to a semicolon-separated string
    def emb_to_str(emb):
        # Format floats to 8 decimals
...
def create_neptune_db_data(
    ...
    graph_path = os.path.join(output_prefix)
    ...
    print("Converted raw graph data into Neptune Database import format!")
