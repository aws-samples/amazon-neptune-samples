...
def analyze_neptune_export(
    ...
    files = fs_handler.list_files(r".csv")
    assert len(files) > 0, f"No csv files found in {osp.abspath(input_path)}"
    file_format = InputFormat.CSV if files[0].endswith(".csv") else InputFormat.PARQUET

    logging.info(f"Found {len(files)} {file_format.value} files")
        ...
        for relation_str in relation_types:
            ...
            if "," in relation_str:
                ...
                if {src_type, dst_type}.issubset(vertex_types):
                    ...
                    # we use the fully qualified relation as the name of the relation type
                    relation_name = relation_str
            ...
            if len(parts) == 3:
                ...
                if {src_type, dst_type}.issubset(vertex_types):
                    ...
                    logging.warning(
                        ...
                        f"Falling back to manual input."
                    )
            else:
                logging.warning(
...
def suggest_transformations(
    ...
    for etype, _ in relationships.items():
        ...
        # First try extracted relation name
        if relation in relation_to_schema:
            etype_info = relation_to_schema[relation]
        # If not there try the full triple as relation name
        elif ",".join(etype) in relation_to_schema:
            etype_info = relation_to_schema[relation]
        # Finally try finding in the original edge_schemas
            ...
            logging.warning(
                ...
                f"and : {list(relation_to_schema.keys())}"
            )
            continue
...
def create_graphstorm_config(
    ...
    add_reversed_edges: bool
        When true will add one reversed edge type for every edge in the graph
    ...
    print("Created GraphStorm configuration dictionary")
    return gs_config
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "When provided will add a reverse edge type for every edge type in the output config."
        ),
    )
...
def main():
    ...
    gs_config = create_graphstorm_config(
        ...
        add_reversed_edges=args.add_reversed_edges,
    )
